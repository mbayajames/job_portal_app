...
class PaymentService {
  ...
  static Future<String> pollPaymentStatus(
    ...
    for (int attempt = 0; attempt < maxAttempts; attempt++) {
      final statusResult = await getTransactionStatus(checkoutRequestId: transactionId);
      if (statusResult != null && statusResult['success'] == true) {
        final status = statusResult['data']['status'].toString().toLowerCase();
  ...
  static Future<Map<String, dynamic>?> checkPaymentStatus(String transactionId) async {
    return await getTransactionStatus(checkoutRequestId: transactionId);
  }

  // Verify payment (check if completed)
  static Future<bool> verifyPayment(String paymentId) async {
    final statusResult = await getTransactionStatus(checkoutRequestId: paymentId);
    if (statusResult != null && statusResult['success'] == true) {
      final status = statusResult['data']['status'].toString().toLowerCase();
